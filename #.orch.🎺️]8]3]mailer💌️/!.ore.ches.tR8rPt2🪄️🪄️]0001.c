...
//⁉️ are they supposed 2 be same size ? padd/cutt? (not yet🗜️✳️
  ...
  system("./+x/4.mMHA]VA5]1]0001.+x -1 f$+/q_2.txt -2 f$+/k_2.txt -3 f$+/v_2.txt -4 f$+/mha_2.txt -5 f$+/resultant_2.txt -+");
...
printf(":../+x/17.pad_cut]TOOL]2]FULL+.c.+x\n");  //.txt only
 ...
 //🔦️ FIX STARTS HERE ! 
   ...
   // i changed this from comparing 2 2a 2 0b... makes sense? is our io right? 
    printf(":./+x/4.encode_qkv]32]de.+x✔️\n");  //.txt only
  system("./+x/4.encode_qkv]32]de.+x -1 f$+/add_n_norm_0b.txt -2 f$+/q_2b.txt -3 f$+/k_2b.txt -4 f$+/v_2b.txt -5 f$+/pad_cut_v_2.txt");  //.txt only
   

 // no mask
   printf(":./+x/4.mMHA]VA5]1]0001.+x✔️\n");  //.txt only
  system("./+x/4.mMHA]VA5]1]0001.+x -1 f$+/q_2b.txt -2 f$+/k_2b.txt -3 f$+/v_2b.txt -4 f$+/mha_2b.txt -5 f$+/pad_cut_v_2.txt --");
  //🤔️ y is this shorter than "corpus length? i added padding 
     //🤔️wuts truly going on here? a mistake? 
     
 printf(":./+x/5.add_n_norm]+]0000.+x✔️\n");  //.txt only
  system("./+x/5.add_n_norm]+]0000.+x -1 f$+/mha_2b.txt -2 f$+/pad_cut_v_2.txt -3 f$+/add_n_norm_2a.txt");
  ////
...
// yes i will just fork "llout here 
  ...
  printf(":./+x/19.word_probe]30]fix🥽️]0003.+x✔️\n");  //.txt only
  system("./+x/19.word_probe]30]fix🥽️]0003.+x -1 f$+/index_0.txt -2 f$+/add_n_norm_2b.txt -3 f$+/weights_matrix_0.txt -4 f$+/answer_i.txt");
