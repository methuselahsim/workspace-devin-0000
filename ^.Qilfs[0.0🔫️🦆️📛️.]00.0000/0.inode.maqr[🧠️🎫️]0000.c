...
int read_iblox_count = Std_read_iblox_count_H(blockfilename_Array_H[0]);
...
Std_inc_iblox_count_H(blockfilename_Array_H[0]);


printf("read_block_count= %d\n",read_block_count);
printf("read_iblox_count=  %d\n",read_iblox_count);

//Std_register_block_Inode(); //📌️
     int realInodeCount = Std_read_FD_POS_count_H(std_superBLockFilename_H, std_iNodeCountOffset_H,STD_txtFS_SB_fieldlength_bin_H);
     
     realInodeCount = Std_inc_FD_POS_count_H(std_superBLockFilename_H, std_iNodeCountOffset_H,STD_txtFS_SB_fieldlength_bin_H);
