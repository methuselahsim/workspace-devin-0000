...
//4	4	s_blocks_count  //THESE ARE SAME THING TBH USE ONLY 1
//8	4	s_r_blocks_count //THESE ARE SAME THING TBH USE ONLY 1
printf("local_BlockCountBinary)🧟️➕️➕️\n");
char *local_BlockCountBinary = malloc(STD_txtFS_SB_fieldlength_bin_H);
...
printf("obv breaks here)🧟️➕️➕️\n"); // apperantly breaks in old code 2.
// none of my code will inode past 16 tho i coulda swore it used  2...:/ 
// NONE OF IT 🧟️ WTF? W/E 
...
printf("⚙️binBuff = %s \n📌️",binBuff );//✅️
//readFrom_Position( 10, local_AsciiBuffer_1 ,binBuff, stdKbdAsciiSize_H); 
  //  printf("🥷️readFrom_Position⚙️ = %d, %s\n",i,local_AsciiBuffer_1 );
int fd ; 
char *stdBlockBuffer_1 = malloc(stdBlockSize_H );
...
long long unsigned int localDropCount = 0;
 int global_inode_ct = 1 ;  //
  int global_ezPos_Iwise = 0 ; // can make more robust later.(LOAD FILE OFC)
   ...
   printf("⚙️bytecount = %d \n📌️",bytecount );//✅️
      char *local_AsciiBuffer_1 = malloc(stdKbdAsciiSize_H);  // = readfrom
  int i;
 ...
 write2Position_0000(i * stdKbdAsciiSize_H ,stdBlockBuffer_1,local_AsciiBuffer_1,stdKbdAsciiSize_H); 
 printf("stdBlockBuffer_1🧟️📌️ = %s\n", stdBlockBuffer_1);//🧟️📌️
//if((localDropCount * stdKbdAsciiSize_H) > (stdBlockSize_H * global_inode_ct) -1 ){


if((i * stdKbdAsciiSize_H) > (stdBlockSize_H * global_inode_ct) -1 ){
 printf("🥷️in if!?!=  📌️ 📌️\n");
//📌️ pin 
//📌️ 📌️ 📌️ 📌️ 📌️ 📌️ 📌️ 📌️ 📌️ 4.12.24
global_inode_ct = stdIncInodeAddBloc_H(filename_arg);// + 1 ;
 printf("⛳️ made it out of stdIncInodeAddBloc_H⛳️ \n");
  printf("🥷️TEST:new global_inode_ct = %d\n",global_inode_ct );
printf("🥷️TEST:new filename_arg(is new inode)🖨️📌️ = %s\n", filename_arg);//🧟️📌️ Std_inc_inode_count_H(); < trying this instead
//this "SHOULD be taking arg now? of wut? 
//🧟️
///4.8.24 
// should be done. a few args added etc. nothing u cant grok.
//////////////🧟️

// 📌️ (it keeps making new blocks (NO!) wut IS count(console.log?
//return 0;
//🌐️
//🤔️
}else{
...
//////////🔄️❌️(i removed this cuz we dont need 2 check file each cycle {and it confusing}
/////////🔄️❌️ofc in mem data is diff cuz multi player. (so maybe we add it back w/e) 
...
int subSum = (global_inode_ct -1 )  * (stdBlockSize_H  )  ;
...
//sprintf("inode_ct = %d", global_inode_ct);
strcpy(stdCommentString_0,buffer1);//📟️	🖨️
*/

// 2 do this right a need a readout of "write2pos*16 each time like inodect)📌️
///
