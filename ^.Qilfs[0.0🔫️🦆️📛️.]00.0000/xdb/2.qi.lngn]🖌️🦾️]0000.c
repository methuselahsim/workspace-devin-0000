...
// (i think the count that got us here may need 2 be a multiplier...
//(of "inode block count...(where is that ? in inode header?) 
//✳️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️🧕️
//__________________
//______krufty at this point?
///save out = (_) ? 
//2do📅️3.25.24 = 
//📌️ pin 

//______prev 2 do 
//📅️2do = make new bl0c.pro (skip kdb2mem4now<3)
//📅️  register it in "inode"? whatever that is🎁️
//>the first (2nd "bl0c.pro" made should already be  
// first inode block. *ignore making kbd2mem inode 
// just dump based on inode bitmap count or w/e
//(so we will actually use "3rd made should fill out
//2nd as an inode , maybe even "inode maker 
// instead of block maker, where u pass block into
// inode aftermaking it or w/e
//
//(yes 4+ fx() "Make_Inode()" "makeBlock()"
//"incBlockcount()"
 //addblock2inode() +anything else this needs 

//🎁️ > filename.txt (

//🎁️ > bitcount (not counting 0tailpad,duh)
		//will autofill 4 load,duh)
	//+optional (filetype/timestamp/perms/etc
//🎁️ > direct pointer (x12)
//🎁️ > single indirect pointer  (13)
//🎁️ > double indirect pointer (14)
//🎁️ > triple indirect pointer  (15 final)
// if a block is "all 0 " its "END"(not SB)
//https://flylib.com/books/en/2.849.1.103/1/  📦️
//https://cscie28.dce.harvard.edu/lectures/lect04/6_Extras/ext2-struct.html#I-BLOCK   📦️//
//meaning we 'immediately associate 'block-101'or w/e
// with an inode (first inode ever)
// give it a "usr0000_fs/default_0000.txt" and get going
//🎁️ = ONE filename.txt has 1 inode.(which may have however many bl0cs.pro it needs...
//_______KRUFT?
 //📦️ hash/uuid of blox ? (if filename(ONLY) changes?
 // if they change its a new file...this isn't GIT
 // at least not yet


/*
int position = localDropCount * stdKbdAsciiSize_H - localBitmapCount * stdBlockSize_H ;
*/
//💝️
//👩🏻‍🔬️
//smith = BEST & AMAZING
...
//A 1.44 MB floppy has one group descriptor only,

//_______backup, dont need ;)
//https://www.kernel.org/doc/html/latest/filesystems/ext4/group_descr.html?highlight=inode
//💝️
//]3.12.24 >> u are ready 2 make inodes. 
// just make a new file and put it in inode 
//(u can figure out when 2 add first inode 2
//💝️
